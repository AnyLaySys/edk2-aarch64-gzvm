...
STATIC CONST COMPATIBILITY_STR  GicItsCompatibleStr[] = {
  { "arm,gic-v3-its" }
};

STATIC CONST COMPATIBILITY_INFO  GicItsCompatibleInfo = {
  ARRAY_SIZE (GicItsCompatibleStr),
  GicItsCompatibleStr
};
  ...
  )
{
  ...
  for (Index = 0; Index < GicItsNodeCount; Index++) {
    ...
    if (!FdtNodeIsCompatible (Fdt, GicItsNode, &GicItsCompatibleInfo)) {
      continue;
    }
    ...
    ItsIdentifier.ItsId = GicItsIndex;
