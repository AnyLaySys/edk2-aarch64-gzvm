...
//
// GZVM: When EDKII_IOMMU_PROTOCOL is available, the framebuffer must be
// allocated from shared DMA memory so the host (QEMU) can read pixel data.
// Regular AllocateReservedPages returns protected guest RAM, which is
// host-inaccessible after VM_START and results in a blank display.
//
  ...
  )
{
  ...
  //
  // GZVM: try to allocate framebuffer from shared DMA memory via IoMmu.
  // The host needs to read pixel data, which is impossible from protected RAM.
  //
  FbBase = 0;
  ...
  if (!EFI_ERROR (Status) && mIoMmu != NULL) {
    ...
    if (!EFI_ERROR (Status) && FbPtr != NULL) {
      ...
      DEBUG ((
        DEBUG_INFO,
        "Ramfb: Framebuffer allocated from IoMmu shared pool at 0x%lx\n",
        (UINT64)FbBase
        ));
    } else {
      DEBUG ((
