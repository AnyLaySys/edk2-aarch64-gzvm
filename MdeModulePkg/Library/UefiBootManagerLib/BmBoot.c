...
STATIC CONST CHAR16                          *mBmFallbackBootFileNames[] = {
  EFI_REMOVABLE_MEDIA_FILE_NAME,
  L"\\EFI\\BOOT\\GRUBAA64.EFI",
  L"\\EFI\\MANJARO\\GRUBAA64.EFI",
...
STATIC
EFI_DEVICE_PATH_PROTOCOL *
BmGetNextBootFilePath (
  IN     EFI_HANDLE                Handle,
  IN     EFI_DEVICE_PATH_PROTOCOL  *FullPath,
  IN OUT BOOLEAN                   *GetNext
  )
{
  EFI_DEVICE_PATH_PROTOCOL  *NextFullPath;
  UINTN                     FileNameIndex;

  for (FileNameIndex = 0; FileNameIndex < ARRAY_SIZE (mBmFallbackBootFileNames); FileNameIndex++) {
    NextFullPath = FileDevicePath (Handle, mBmFallbackBootFileNames[FileNameIndex]);
    if (*GetNext) {
      return NextFullPath;
    }

    if ((GetDevicePathSize (NextFullPath) == GetDevicePathSize (FullPath)) &&
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    return BmGetNextBootFilePath (Handle, FullPath, &GetNext);
  }
  ...
  for (Index = 0; Index < NumberSimpleFileSystemHandles; Index++) {
    ...
    if ((Size <= TempSize) && (CompareMem (TempDevicePath, DevicePath, Size) == 0)) {
      NextFullPath = BmGetNextBootFilePath (SimpleFileSystemHandles[Index], FullPath, &GetNext);
      if (NextFullPath != NULL) {
        break;
      }
    }
