...
STATIC CHAR16  mGzvmExtlinuxCommandLine[] =
  L"initrd=\\initramfs-linux.img root=PARTUUID=d17455d7-ee76-4831-83c8-5bbe6e8eb5bc rw rootwait audit=0 acpi=force console=ttyAMA0,115200 earlycon=pl011,0x09000000 earlyprintk=serial,pl011,0x09000000 keep_bootcon ignore_loglevel loglevel=8 efi=debug";

STATIC CONST CHAR8  mGzvmExtlinuxAsciiCommandLine[] =
  "initrd=\\initramfs-linux.img root=PARTUUID=d17455d7-ee76-4831-83c8-5bbe6e8eb5bc rw rootwait audit=0 acpi=force console=ttyAMA0,115200 earlycon=pl011,0x09000000 earlyprintk=serial,pl011,0x09000000 keep_bootcon ignore_loglevel loglevel=8 efi=debug";
  ...
  )
{
  ...
  if (Node < 0) {
    Node = FdtAddSubnode (Dtb, 0, "chosen");
  }
  ...
  FdtSetProp (Dtb, Node, "bootargs", mGzvmExtlinuxAsciiCommandLine, sizeof (mGzvmExtlinuxAsciiCommandLine));
  FdtSetProp (Dtb, Node, "stdout-path", "/pl011@9000000", sizeof ("/pl011@9000000"));
}

STATIC
BOOLEAN
BdsBootExtlinuxImage (
  VOID
  )
{
  EFI_STATUS                    Status;
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    FilePath = FileDevicePath (Handles[Index], L"\\Image");
    if (FilePath == NULL) {
      continue;
    }
    ...
    Status = EfiBootManagerInitializeLoadOption (
               ...
               L"Manjaro ARM Linux",
               FilePath,
               (UINT8 *)mGzvmExtlinuxCommandLine,
               sizeof (mGzvmExtlinuxCommandLine)
               );
    FreePool (FilePath);
    if (EFI_ERROR (Status)) {
      continue;
    ...
    BootSuccess = (BOOLEAN)(BootOption.Status == EFI_SUCCESS);
  ...
  )
{
  ...
  if (!PlatformRecovery) {
    ...
    do {
      ...
      //
      BootSuccess = BdsBootExtlinuxImage ();
      LoadOptions = EfiBootManagerGetLoadOptions (&LoadOptionCount, LoadOptionTypeBoot);
      if (LoadOptions != NULL) {
        BootSuccess = (BOOLEAN)(BootBootOptions (LoadOptions, LoadOptionCount, (BootManagerMenuStatus != EFI_NOT_FOUND) ? &BootManagerMenu : NULL) || BootSuccess);
        EfiBootManagerFreeLoadOptions (LoadOptions, LoadOptionCount);
      }
